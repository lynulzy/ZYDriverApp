...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    DDLog(@"%@",UDID);
    [ZSXJUserDefaults initUserDefaults];
    [self setupBaiduMap];
//    NSInteger startTime = [[NSDate date] timeIntervalSince1970];
//    NSDateFormatter *df = [[NSDateFormatter alloc] init];
//    df.timeStyle = NSDateFormatterFullStyle;
//    [df setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
//    NSString *time = [df stringFromDate:[NSDate dateWithTimeIntervalSince1970:startTime]];
...
- (void)setupBaiduMap {
    ...
    if (!ret) {
        DDLog(@"Baidu Manager Start failed");
    }
}
...
- (void)applicationWillTerminate:(UIApplication *)application {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:NOTI_NAME_TERMINATE
                                                        object:nil];
}
